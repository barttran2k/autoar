...
class ArachniScanner:
    def __init__(self):
        try:
            status_ar = subprocess.check_output(
                ['./arachni/bin/arachni', '--version']).decode('utf-8')
            match = re.search(r'Arachni ([\d\.]+)', status_ar)
            self.arachni_version = ""
            if match:
                self.arachni_version = "Arachni version: " + \
                    Fore.YELLOW+(match.group(1))
            else:
                self.arachni_version = "Arachni not found"
        except:
            self.arachni_version = "Arachni not found"
    ...
    def scanSQL(self):
        print(self.arachni_version + Style.RESET_ALL)
        # Nhập url cần scan
        url = input("Nhập URL: ")
        while True:
        ...
        if input() == "y":
            ...
            while True:
                if self.is_valid_url(url):
                    ...
                    print(Fore.RED + "Invalid URL..." + Style.RESET_ALL)
                    urllogin = input(
                        Fore.YELLOW + "Input URL: " + Style.RESET_ALL)
            parameter = input(
                Fore.YELLOW + "Input parameter: " + Style.RESET_ALL)
            check = input(Fore.YELLOW + "Input check: " + Style.RESET_ALL)
            autoLogin = '--plugin=autologin:url=' + urllogin + \
                ',parameters="' + parameter + '",check="' + check + '" '
